...
function buildInventoryCsv(groups: ItemGroup[], nicheName: string): string {
  ...
  const rows = groups.flatMap((g) =>
    g.items.map((it: ResolvedItem) =>
      [
        layer[g.key] || g.key,
        it.item,
        it.examples,
        it.dataSubjects,
        it.processingPurposes,
        it.sources,
  ...
  const title = `# DPDPA personal data inventory — ${nicheName} — generated by SaralPrivacy Discovery`;
  return [title, header.map(esc).join(","), ...rows].join("\r\n");
}
...
export default function ResultPanel({
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        a.href = url;
        a.download = `${nicheId}-dpdpa-data-inventory.csv`;
  ...
  return (
    <div className="result">
      ...
      {/* Lead-gated CSV data-inventory download → reuses /api/template-download.
