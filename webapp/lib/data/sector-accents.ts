...
export interface SectorAccent {
  ...
  hue: string;
  /**
   * The dark ramp's `icon` shade (hue-300) as a literal hex, for surfaces that
   * cannot use Tailwind classes — the server-rendered share images. Brand green
   * and teal are this repo's own @theme values; the other hues are Tailwind
   * v4's oklch 300 shades converted to sRGB. Every one clears 8.6:1 on
   * navy-700 #121A2E (measured).
   */
  hex: string;
  /**
