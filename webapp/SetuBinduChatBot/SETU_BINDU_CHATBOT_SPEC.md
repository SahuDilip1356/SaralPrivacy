...
| D3 | Model | **`claude-sonnet-5`** (supersedes the `claude-sonnet-4-6` lock of July). Set in ONE place: `CHAT_MODEL` in `lib/chat/system-prompt.ts`. ⛔ Changing it invalidates the red-team result — injection resistance is a model property, so `scripts/redteam-chat.mts` must pass against a preview before any new value ships. **Haiku 4.5 was tried 2026-08-07 and reverted on evidence: it adopted the A3 injected claim in ~8% of runs.** |
